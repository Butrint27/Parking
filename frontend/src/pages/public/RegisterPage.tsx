...
const RegisterPage = () => {
  ...
  className='flex-1 min-h-[400px] h-4/5 bg-[#f0ecf7] flex flex-col justify-center items-center rounded-r-2xl p-4 max-w-sm'
>
  <h1 className='text-3xl font-bold mb-4 text-[#754eb4]'>Register</h1>
  ...
  <InputField control={control} label='Email Required' inputName='email' error={errors.email?.message} />
  <InputField
    control={control}
    label='Password'
    inputName='password'
  ...
  <div className='px-2 mt-2 mb-4 w-full flex gap-2 justify-center'>
    <h1 className='text-sm'>Already Have an account?</h1>
    <Link
      to={PATH_PUBLIC.login}
      className='text-[#754eb4] border border-[#754eb4] hover:shadow-[0_0_5px_2px_#754eb44c] px-3 py-1 text-sm rounded-2xl duration-200'
    >
      Log in
    </Link>
  </div>
